...
def load_data():
    ...
    count_req = f'https://data.epa.gov/efservice/FRS_INTEREST/STATE_CODE/=/{state_code}/COUNT'
...
def clean_attributes(facilities):
    ...
    fac['interest_type'] = fac['interest_type'].apply(lambda x: (''.join(((word if word in ['NSR', 'NPDES', 'ICIS', 'OSHA', 'ICIS-', 'CESQG', 'SQG', 'AFO','SW', 'BRAC', 'CZM', 'EPCRA', 'FRP', 'LQG', "II", "WIPP", 'NPL', 'TRI', 'TSCA', 'TSD', "UIC", 'VSQG', 'NESHAPS', 'SPCC'] else word.title()) for word in x.translate(replacements).split()))) if not pd.isna(x) else None)
...
def get_iris(facility):
    ...
    program = epa_frs_data[f"d.ProgramInformationSystem.{facility['pgm_sys_acrnm']}"] #this can different from interest program
    ...
    return facility_iri,  record_iri, program, interest
...
def triplify(facilities):
    ...
    for idx, facility in facilities.iterrows():
        #get iris
        facility_iri, record_iri, program, interest = get_iris(facility)

        #create facility
        kg.add((facility_iri, RDF.type, epa_frs["FRS-Facility"]))
        #kg.add((facility_iri, RDF.type, facility_class))
        kg.add((facility_iri, epa_frs['hasRecord'], record_iri))

        #Monitoring record
        kg.add((record_iri, epa_frs['fromSystem'], program))
        kg.add((record_iri, DCTERMS.identifier, Literal(facility.pgm_sys_id, datatype=XSD.string)))
        if pd.notnull(facility.start_date):
            kg.add((record_iri, PROV.startedAtTime, Literal(facility['start_date'].strftime('%Y-%m-%dT%H:%M:%S') , datatype=XSD.dateTime)))
            if facility.start_date_qualifier == 'CLS':
                kg.add((record_iri, PROV.endedAtTime, Literal(facility['start_date'].strftime('%Y-%m-%dT%H:%M:%S') , datatype=XSD.dateTime)))
        if pd.notnull(facility.end_date):
            kg.add((record_iri, PROV.endedAtTime, Literal(facility['end_date'].strftime('%Y-%m-%dT%H:%M:%S') , datatype=XSD.dateTime)))
        if pd.notnull(facility.create_date):
            kg.add((record_iri, DCTERMS.created, Literal(facility['create_date'].strftime('%Y-%m-%dT%H:%M:%S') , datatype=XSD.dateTime)))
        if pd.notnull(facility.update_date):
            kg.add((record_iri, DCTERMS.modified, Literal(facility['update_date'].strftime('%Y-%m-%dT%H:%M:%S') , datatype=XSD.dateTime)))
    
        if 'class' in interest.keys():
            #environmental Interest
            kg.add((record_iri, epa_frs['ofInterestType'], interest['class']))

            if pd.notnull(facility.interest_description) and facility.interest_description != "":
                kg.add((record_iri, DCTERMS.description, Literal(facility['interest_description'], datatype=XSD.string)))
            if pd.notnull(facility.interest_label) and facility.interest_label != "":
                kg.add((record_iri, RDFS.label, Literal(facility.interest_label, datatype=XSD.string)))
