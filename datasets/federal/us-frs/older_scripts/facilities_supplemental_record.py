...
def load_data():
    ...
    while True: #limit < facilities_count+increment:
        facilities_url = f'https://data.epa.gov/efservice/frs.frs_supplemental_interest/pgm_sys_acrnm/equals/{program}/{start}:{limit}/json' #join/frs.frs_supplemental_interest/
...
def clean_attributes(facilities):
    ...
    replacements = str.maketrans({"(":"- ",
                     ...
                     ",":"",
                     ":":"-"})
    fac['interest_type'] = fac['sup_interest_type'].apply(lambda x: (''.join(((word if word in ['NSR', 'NPDES', 'ICIS', 'OSHA', 'ICIS-', 'CESQG', 'SQG', 'AFO','SW', 'BRAC', 'CZM', 'EPCRA', 'FRP', 'LQG', "II", "WIPP", 'NPL', 'TRI', 'TSCA', 'TSD', "UIC", 'VSQG', 'NESHAPS', 'SPCC'] else word.title()) for word in x.translate(replacements).split()))))
    return fac

def get_iris(facility):
    iris = {}
    iris['monitoring'] = epa_frs_data[f"d.MonitoringRecord.{facility['pgm_sys_id']}"]
    if pd.notnull(facility['sup_pgm_sys_id']):
         iris['supplemental'] = epa_frs_data[f"d.SupplementalRecord.{facility['sup_pgm_sys_acrnm']}.{facility['sup_pgm_sys_id']}"]
    else:
         iris['supplemental'] = epa_frs_data[f"d.SupplementalRecord.{facility['sup_pgm_sys_acrnm']}.{facility['sup_interest_id']}"]
    iris['program'] =  epa_frs[f"d.Program.{facility['sup_pgm_sys_acrnm'].upper()}"] 
...
def triplify(facilities):
    ...
    for idx, facility in facilities.iterrows():
        ...
        kg.add((iris['monitoring'], epa_frs['hasSupplementalRecord'], iris['supplemental']))
        kg.add((iris['supplemental'], RDF.type, epa_frs['SupplementalRecord']))
        if pd.notnull(facility['sup_pgm_sys_id']):
             kg.add((iris['supplemental'], DCTERMS.identifier, Literal(facility['sup_pgm_sys_id'])))

        if pd.notnull(facility.start_date):
                kg.add((iris['supplemental'], PROV.startedAtTime, Literal(facility['start_date'].strftime('%Y-%m-%dT%H:%M:%S') , datatype=XSD.dateTime)))
                if facility.start_date_qualifier == 'CLS':
                    kg.add((iris['supplemental'], PROV.endedAtTime, Literal(facility['start_date'].strftime('%Y-%m-%dT%H:%M:%S') , datatype=XSD.dateTime)))
        if pd.notnull(facility.end_date):
                kg.add((iris['supplemental'], PROV.endedAtTime, Literal(facility['end_date'].strftime('%Y-%m-%dT%H:%M:%S') , datatype=XSD.dateTime)))
        if pd.notnull(facility.create_date):
                kg.add((iris['supplemental'], DCTERMS.created, Literal(facility['create_date'].strftime('%Y-%m-%dT%H:%M:%S') , datatype=XSD.dateTime)))
        if pd.notnull(facility.update_date):
                kg.add((iris['supplemental'], DCTERMS.modified, Literal(facility['update_date'].strftime('%Y-%m-%dT%H:%M:%S') , datatype=XSD.dateTime)))

        kg.add((iris['supplemental'], epa_frs['ofInterestType'], iris['interest']))
        kg.add((iris['supplemental'], epa_frs['fromSystem'], iris['program']))
    return kg
...
def main():
    ...
    kg = triplify(data)
    if testing:
        kg_turtle_file = output_dir/ f"epa-frs-data-facility-{program}-supplemental-test.ttl"
    else:
        kg_turtle_file = output_dir / f"epa-frs-data-facility-{program}-supplemental.ttl"
    kg.serialize(kg_turtle_file, format='turtle')
    logger = logging.getLogger(f'Finished triplifying {program} program facilities supplemental.')
